...
export const ProductCardUI = ({ product }: ProductCardUIProps) => {
  return (
    <HeadlessProductCard product={product}>
      {(logic) => (
        <Card className="bg-card border border-white/10 hover:border-white/30 transition-all duration-300 group overflow-hidden">
              ...
              <div className="aspect-square bg-secondary/30 overflow-hidden relative">
                ...
                <div className="absolute top-3 left-3 flex flex-col gap-1">
                  {logic.discountPercentage && (
                    <span className="glass text-primary text-xs px-2 py-1 font-mono tracking-wider">
                      -{logic.discountPercentage}%
                    </span>
                  )}
                  ...
                  {!logic.inStock && (
                    <span className="glass text-muted-foreground text-xs px-2 py-1 font-mono tracking-wider">
                      OUT OF STOCK
                    </span>
                  )}
                </div>
              ...
              {logic.hasVariants && logic.options && (
                <div className="mb-3 space-y-2">
                  {logic.options.map((opt) => (
                    <div key={opt.id}>
                      <div className="text-xs font-mono text-muted-foreground mb-1">{opt.name}</div>
                              ...
                              className={`border rounded px-2 py-1 text-xs font-mono ${
                                isSelected 
                                  ...
                                  : logic.selected[opt.name] && !isSelected
                                    ...
                                    : 'border-border bg-transparent text-foreground hover:border-white/30'
                              }`}
